...
const handleBrowserProxy = async (req, res) => {
  try {
    ...
    if (!targetUrl) {
      return res.send(`
        ...
        <head>
          ...
          <style>
            body { background: #0f172a; color: #f8fafc; font-family: system-ui, sans-serif; height: 100vh; margin: 0; display: flex; align-items: center; justify-content: center; text-align: center; }
            .card { background: #1e293b; border: 1px solid #334155; border-radius: 1rem; padding: 2.5rem; max-width: 480px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.5); }
        ...
        <body>
          <div class="card">
            <h2>🌐 WebOS Chrome Tarayıcı</h2>
            <p>Aramak istediğiniz terimi veya gitmek istediğiniz web adresini yukarıdaki adres çubuğuna yazarak <b>Git</b> butonuna basın.</p>
          </div>
        </body>
        </html>
      `);
    ...
    if (contentType.includes('text/html')) {
      ...
      // Remove security restriction meta tags
      html = html.replace(/<meta[^>]*http-equiv=["']?content-security-policy["']?[^>]*>/gi, '');
      html = html.replace(/<meta[^>]*http-equiv=["']?X-Frame-Options["']?[^>]*>/gi, '');

      // Inject base URL tag so all relative scripts, stylesheets, and images load correctly
    ...
    res.status(500).send(`
      ...
      <head>
        ...
        <style>
          ...
          .err-box { max-width: 500px; margin: 2rem auto; background: #1e293b; padding: 2rem; border-radius: 1rem; border: 1px solid #334155; }
          h2 { color: #f43f5e; margin-top: 0; font-size: 1.25rem; }
          p { color: #94a3b8; font-size: 0.875rem; }
          code { background: #0f172a; padding: 0.5rem 0.75rem; border-radius: 0.5rem; color: #38bdf8; display: block; margin: 1rem 0; word-break: break-all; font-family: monospace; font-size: 0.8rem; }
        </style>
      </head>
      <body>
        <div class="err-box">
